...
{
    public partial class AlterDailySummarizeTransactionsStoredProcedure_202311080727 : Migration
    {
...
-- =============================================
-- exec Report_Daily_Summarize_Transactions @date = '2023-11-08', @userId = 3
...
BEGIN
    ...
    ;WITH ProductsValueAmount AS (
        SELECT 
            SUM(ROUND(COALESCE(pp.StockQuantity, 0) * p.PricePerUnit, 2)) AS Amount
        FROM 
            Products p
    ...
    SELECT 
        CAST((p.Amount + pt.Amount) AS DECIMAL(10, 2)) AS InitialStocks
        , CAST(p.Amount AS DECIMAL(10, 2)) AS AfterStocks
        , CAST(pt.Amount AS DECIMAL(10, 2)) AS TotalSales
    FROM 
