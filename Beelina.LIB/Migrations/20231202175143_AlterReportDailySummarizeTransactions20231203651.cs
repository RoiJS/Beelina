...
{
    public partial class AlterReportDailySummarizeTransactions20231203651 : Migration
    {
...
BEGIN
	...
	, DiscountedCollectibles AS (
		SELECT 
			c.Id
			, CAST(c.Collectibles AS DECIMAL(10, 2)) AS GrossCollectibles
			, CAST(c.Discount AS DECIMAL(10, 2)) AS Discount
			, CAST(SUM(c.Collectibles - ((c.Discount / 100) * c.Collectibles)) AS DECIMAL(10, 2)) AS NetCollectibles
			, c.DueCollectibles
			, c.StoreName
			, c.StoreAddress
			, c.OutletType
	...
	SELECT 
		...
		, c.OutletType
		, c.PaymentMethod
		, c.OrderReceived AS DateCreated
