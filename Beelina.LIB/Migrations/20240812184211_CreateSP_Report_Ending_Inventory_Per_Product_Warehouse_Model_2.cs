...
{
    /// <inheritdoc />
    public partial class CreateSP_Report_Ending_Inventory_Per_Product_Warehouse_Model_2 : Migration
    {
        /// <inheritdoc />
...
-- =============================================
CREATE OR ALTER     PROCEDURE [dbo].[Report_Ending_Inventory_Per_Product_Warehouse_Model_2]
...
BEGIN
	...
	;WITH PreviousOrderTransactionsPerProduct AS (
		...
		FROM
			dbo.TVF_OrderTransactions(@userId, @warehouseId, 2) ot
		WHERE
	...
	, EndingStocksPerWarehouseProductReport AS (
		...
		SELECT
			...
			, (COALESCE(pp.RemainingStock, 0) + COALESCE(c.Stock, 0) - COALESCE(cp.SoldQuantity, 0)) * COALESCE(pw.PricePerUnit, 0) AS EndingStocksValue
			--, pp.PricePerUnit
			--, pp.RemainingStock
			--, c.Stock
			--, cp.SoldQuantity
